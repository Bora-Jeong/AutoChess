...
public class GamePanel : PanelBase<GamePanel>
{
    ...
    public void GameOver(bool win, int winCount, int loseCount)
    {
        _winCountText.text = $"{winCount}회";
        _loseCountText.text = $"{loseCount}회";
        StartCoroutine(ResultTextAnimation(win));
    ...
    IEnumerator GameOverCor()
    {
        ...
        while (sec > 0)
        {
            _lobbyText.text = $"{sec}초 뒤 로비로 이동";
            sec--;
    ...
    IEnumerator ResultTextAnimation(bool win)
    {
