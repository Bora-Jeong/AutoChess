...
public class ShopPanel : PanelBase<ShopPanel>
{
    ...
    {
        if (Player.instance.gold < unit.Data.Gold || InventoryManager.instance.IsFull())
        {
    ...
    public void BuyExp()
    {
        if (Player.instance.gold < Constants.requiredGoldToBuyExp || Player.instance.level >= Constants.playerMaxLevel)
        {
            SoundManager.PlaySFX("nogold");
            return;
        }
