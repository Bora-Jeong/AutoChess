...
public class PacketManager : HNET.CHNetConnector
{
    ...
    public void SendPacket_ReportResult(bool win) // 게임 결과
    {
        HNET.NewPacket Out = new HNET.NewPacket((int)PacketType.ReportResult);
        Out.In(Player.instance.nickname, Encoding.Unicode);
        Out.In(win);
        Debug.Log($"Send_Request {win}   {Player.instance.nickname}");
