...
public class TableData : Singleton<TableData>
{
    ...
    private Dictionary<int, ItemTableData> _itemTableDic;

    public Dictionary<int, UnitTableData> unitTableDic => _unitTableDic;
    public Dictionary<int, ItemTableData> itemTableDic => _itemTableDic;
    private void Awake()
    {
        LoadTableData();
    }
    ...
    private void LoadTableData()
    {
        ...
        _itemTableDic = new Dictionary<int, ItemTableData>();
        for (int i = 0; i < _itemTable.dataArray.Length; i++)
            _itemTableDic.Add(_itemTable.dataArray[i].Typeid, _itemTable.dataArray[i]);
    }
    ...
    public ItemTableData GetItemTableData(int typeID)
    {
        if (_itemTableDic.TryGetValue(typeID, out ItemTableData data))
            return data;
        Debug.LogError($"TypeID가 {typeID} 인 데이터가 ItemTable에 존재하지 않습니다.");
        return null;
    }
